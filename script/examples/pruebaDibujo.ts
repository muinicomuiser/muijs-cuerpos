...
window.addEventListener("load", ()=>{
    ...
    //Crear Cosas
    let numeroCuerpos: number = 55;
    let poligonoPatron: Forma = Forma.poligono(centroCanvas.x, centroCanvas.y*(1/2), numeroCuerpos, CANVAS.width*0.2);
    let circulitoCentro: Cuerpo = Cuerpo.circunferencia(centroCanvas.x*(7/8), centroCanvas.y*(2/2), CANVAS.width*0.01);
    let circulitoExterno: Cuerpo = Cuerpo.circunferencia(centroCanvas.x*(9/8), centroCanvas.y*(2/2), CANVAS.width*0.01);
    let circulitos: Cuerpo[] = []
    for(let vertice of poligonoPatron.vertices){
        ...
        let circulito: Cuerpo = Cuerpo.circunferencia(vertice.x, vertice.y, circulitoCentro.radio*0.5);
        let velocidad: Vector = Vector.rotar(Vector.segunPuntos(circulito.centro, poligonoPatron.centro), -Matematica.PI*0.5);
    ...
    function prueba(){
        ...
        for(let circulito of circulitos){
            let vectorAtractor: Vector = Vector.segunPuntos(circulito.centro, circulitoCentro.centro);
            vectorAtractor = Vector.normalizar(vectorAtractor);
            let vectorAtractorExterno: Vector = Vector.segunPuntos(circulito.centro, circulitoExterno.centro);
